...
namespace NonVisualComponents.Kokorev
{
    public partial class ComponentExcelReport : Component
    {
        ...
        /// <param name="isHorizontalHead">горизонтальный или вертикальный заголовок</param>
        /// <param name="data">данные для записи</param>
        /// <typeparam name="T">какой то тип</typeparam>
        public void CreateExcelReport<T>(string fileName, bool isHorizontalHead, IEnumerable<T> data)
        {
            using (var spreadsheetDocument = SpreadsheetDocument.Create(fileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                var shareStringPart =
                    spreadsheetDocument.WorkbookPart.GetPartsOfType<SharedStringTablePart>().Any()
                ...
                var fields = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.NonPublic |
                ...
                {
                    var column = GetExcelColumnName(isHorizontalHead ? fieldNumHead : 1);
                    var rowIndex = isHorizontalHead ? 1 : (uint) fieldNumHead;
                ...
                foreach (var dataRow in data)
                {
                    ...
                    foreach (var field in fields)
                    {
                        ...
                        var rowIndex = 0U;
                        column = GetExcelColumnName(isHorizontalHead ? fieldNum : dataRowNum);
        ...
        /// </summary>
        /// <param name="workbookPart">принимаем книгу</param>
        private static void CreateStyles(WorkbookPart workbookPart)
        {
            ...
            var color = new DocumentFormat.OpenXml.Office2010.Excel.Color();
            color.Theme = 1U;
            fontUsual.Append(color);
        ...
        private static void InsertCellInWorksheet(ExcelCellParameters cellParameters)
        {
            ...
            {
                // все ячейки должны быть последовательно друг за другом расположены, что бы понять, куда вставлять
                Cell refCell = null;
            ...
            cell.CellValue = new
                CellValue((cellParameters.ShareStringPart.SharedStringTable.Elements<SharedStringItem>().Count() - 1)
                    .ToString());
